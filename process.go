...
func Process(ch <-chan Sample, conf Config) {
	diff := stats.New[time.Duration](conf.maxSamples, conf.maxSpread, conf.maxInvalidStreak)
	roundtrip := stats.New[time.Duration](conf.maxSamples, conf.maxSpread, conf.maxInvalidStreak)
	i := 1
	for s := range ch {
